...
if django.VERSION < (1, 5):
    from django.utils.safestring import EscapeString as EscapeBytes
    from django.utils.safestring import EscapeUnicode as EscapeText
...
class NonrelDatabaseFeatures(BaseDatabaseFeatures):
    ...
    supports_primary_key_on = set(NonrelDatabaseCreation.data_types.keys()) - set(
        (
            "ForeignKey",
            "OneToOneField",
            "ManyToManyField",
...
class NonrelDatabaseOperations(BaseDatabaseOperations):
    ...
    def check_aggregate_support(self, aggregate):
        ...
        if not isinstance(aggregate, Count):
            raise NotImplementedError("This database does not support %r " "aggregates." % type(aggregate))
    ...
    def value_for_db(self, value, field, lookup=None):
        ...
        if lookup == "isnull":
            ...
            return [self._value_for_db(subvalue, field, field_kind, db_type, lookup) for subvalue in value]
        else:
            return self._value_for_db(value, field, field_kind, db_type, lookup)
    ...
    def _value_for_db(self, value, field, field_kind, db_type, lookup):
        ...
        if field_kind in (
            ...
            value = self._value_for_db_collection(value, field, field_kind, db_type, lookup)

        # Store model instance fields' values.
        elif field_kind == "EmbeddedModelField":
            value = self._value_for_db_model(value, field, field_kind, db_type, lookup)
    ...
    def _value_from_db(self, value, field, field_kind, db_type):
        ...
        if field_kind in (
            ...
            value = self._value_from_db_collection(value, field, field_kind, db_type)

        # Reinstatiate a serialized model.
        elif field_kind == "EmbeddedModelField":
            value = self._value_from_db_model(value, field, field_kind, db_type)

        return value

    def _value_for_db_collection(self, value, field, field_kind, db_type, lookup):
        """
        ...
        """
        subfield, subkind, db_subtype = self._convert_as(field.item_field, lookup)
        ...
        if lookup:
            ...
            if lookup == "exact" and db_type == "list" and value == "[]":
                return []
            value = self._value_for_db(value, subfield, subkind, db_subtype, lookup)
        ...
        # Convert list/set items or dict values.
            ...
            if field_kind == "DictField":
                ...
                value = (
                    (
                        key,
                        self._value_for_db(subvalue, subfield, subkind, db_subtype, lookup),
                    )
                ...
                value = (self._value_for_db(subvalue, subfield, subkind, db_subtype, lookup) for subvalue in value)
    ...
    def _value_from_db_collection(self, value, field, field_kind, db_type):
        ...
        if field_kind == "DictField":
            ...
            return dict((key, self._value_from_db(subvalue, subfield, subkind, db_subtype)) for key, subvalue in value)
        else:

            # Generator yielding deconverted items.
            value = (self._value_from_db(subvalue, subfield, subkind, db_subtype) for subvalue in value)
    ...
    def _value_for_db_model(self, value, field, field_kind, db_type, lookup):
        ...
        value = (
            (
                subfield.column,
                self._value_for_db(subvalue, lookup=lookup, *self._convert_as(subfield, lookup)),
            )
    ...
    def _value_from_db_model(self, value, field, field_kind, db_type):
        ...
        return embedded_model, dict(
            (
                subfield.attname,
                self._value_from_db(value[subfield.column], *self._convert_as(subfield)),
            )
    ...
    def _value_for_db_key(self, value, field_kind):
        ...
        a key itself.
        """
        raise DatabaseError("%s may not be used as primary key field." % field_kind)
