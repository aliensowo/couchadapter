...
class AbstractIterableField(models.Field):
    ...
    def get_db_prep_lookup(self, lookup_type, value, connection, prepared=False):
        ...
        return self.item_field.get_db_prep_lookup(lookup_type, value, connection=connection, prepared=prepared)
...
class DictField(AbstractIterableField):
    ...
    def _map(self, function, iterable, *args, **kwargs):
        return self._type((key, function(value, *args, **kwargs)) for key, value in iterable.iteritems())
...
class EmbeddedModelField(models.Field):
    ...
    def _set_model(self, model):
        ...
        """
        self._model = model
        if model is not None and isinstance(self.embedded_model, (str, bytes)):
    ...
    def get_db_prep_save(self, embedded_instance, connection):
        ...
        if not isinstance(embedded_instance, embedded_model):
            raise TypeError("Expected instance of type %r, not %r." % (embedded_model, type(embedded_instance)))
        ...
        for field in embedded_instance._meta.fields:
            value = field.get_db_prep_save(field.pre_save(embedded_instance, add), connection=connection)
