...
NON_REDIRECTED_BASE_PATHS = tuple(path.rstrip("/") + "/" for path in NON_REDIRECTED_PATHS)
...
class RedirectMiddleware(object):
    ...
    def process_request(self, request):
        ...
        if (
            ...
            or request.path in NON_REDIRECTED_PATHS
            or request.path.startswith(NON_REDIRECTED_BASE_PATHS)
        ):
            return
        if host not in settings.ALLOWED_DOMAINS:
            return HttpResponseRedirect("http://" + settings.ALLOWED_DOMAINS[0] + request.path)
